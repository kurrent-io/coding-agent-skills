// KurrentDB Go Client Example - Append, Read, Subscribe, and Filtered Subscriptions
package main
...
func main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "projection" {
		RunProjection()
		return
	}
		...
		if event.EventAppeared != nil {
			...
			if len(streamID) > 0 && streamID[0] != '$' {
				fmt.Printf("  [Sub] Stream: %s, Type: %s\n",
					streamID,
					event.EventAppeared.OriginalEvent().EventType)
				count++
				if count >= 3 {
					break
				}
	...
	filteredSub, err := client.SubscribeToAll(ctx, kurrentdb.SubscribeToAllOptions{
		From:   kurrentdb.Start{},
	...
	count = 0
	for {
		event := filteredSub.Recv()

		if event.EventAppeared != nil {
			fmt.Printf("  [Filtered] Stream: %s, Type: %s\n",
				event.EventAppeared.OriginalEvent().StreamID,
				event.EventAppeared.OriginalEvent().EventType)
			count++
			if count >= 3 {
				break
	...
	prefixSub, err := client.SubscribeToAll(ctx, kurrentdb.SubscribeToAllOptions{
		From: kurrentdb.Start{},
	...
	count = 0
	for {
		event := prefixSub.Recv()

		if event.EventAppeared != nil {
			fmt.Printf("  [Prefix Filter] Stream: %s, Type: %s\n",
				event.EventAppeared.OriginalEvent().StreamID,
				event.EventAppeared.OriginalEvent().EventType)
			count++
			if count >= 2 {
	...
	eventTypeSub, err := client.SubscribeToAll(ctx, kurrentdb.SubscribeToAllOptions{
		From: kurrentdb.Start{},
		Filter: &kurrentdb.SubscriptionFilter{
	...
	count = 0
	for {
		event := eventTypeSub.Recv()

		if event.EventAppeared != nil {
			fmt.Printf("  [Event Type Filter] Stream: %s, Type: %s\n",
				event.EventAppeared.OriginalEvent().StreamID,
				event.EventAppeared.OriginalEvent().EventType)
			count++
			if count >= 2 {
