#!/usr/bin/env python3
"""KurrentDB Python Client Example - Append, Read, Subscribe, and Filtered Subscriptions"""
...
# === FILTERED SUBSCRIPTION (exclude system events - default behavior) ===
log("\nStarting filtered subscription to $all (excluding system events)...")

# Default filter_exclude already excludes system events by event type
subscription = client.subscribe_to_all(commit_position=0)

count = 0
for event in subscription:
    # Default filtering excludes system events, but let's also skip system streams
    if not event.stream_name.startswith("$"):
        log(f"  [Filtered] Stream: {event.stream_name}, Type: {event.type}")
        count += 1
        if count >= 3:
            break
...
# Use client-side filtering for stream prefix
subscription = client.subscribe_to_all(commit_position=0)

count = 0
for event in subscription:
    if event.stream_name.startswith("order-"):
        log(f"  [Prefix Filter] Stream: {event.stream_name}, Type: {event.type}")
        count += 1
        if count >= 2:
            break
...
# Use client-side filtering for event type prefix
subscription = client.subscribe_to_all(commit_position=0)

count = 0
for event in subscription:
    if event.type.startswith("Order"):
        log(f"  [Event Type Filter] Stream: {event.stream_name}, Type: {event.type}")
        count += 1
        if count >= 2:
            break
...
# Close client connection to allow clean exit
client.close()
